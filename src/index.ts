    ...
    this.server = new Server(
      ...
      },
      {
        capabilities: {
          tools: {},
          resources: {},
          // Add sampling capability to allow the server to request AI generations
          sampling: {},
        },
      }
    );

    // Then create the handlers with the initialized server
    this.commitToolHandlers = new CommitToolHandlers(this.gitOps, this.server);
  ...
  private setupToolHandlers() {
    ...
    this.server.setRequestHandler(ListToolsRequestSchema, async () => ({
      tools: [
        {
          name: "commit",
          description: `ANALYZE GIT CHANGES AND GENERATE COMMIT MESSAGE USING AI.
          
          This tool will:
          1. Analyze staged changes in your git repository
          2. Request AI to generate a commit message based on the diff and your preferences
          3. Validate the generated message
          4. Optionally commit the changes automatically
          ...
          inputSchema: {
            ...
            properties: {
              request: {
                type: "string",
                description:
                  "User's exact words. Do not interpret - use keyword matching only.",
              },
              style: {
                type: "string",
                enum: ["default", "minimal", "descriptive", "strict"],
                description:
                  "Style based on interpreted user intent, Default if no keywords.",
                default: "default",
              },
              auto_commit: {
                type: "boolean",
                default: true,
                description:
                  "MUST be true unless user explicitly says 'don't commit' or similar",
              },
              auto_stage: {
                type: "boolean",
                default: true,
                description:
                  "Always true unless user explicitly mentions staging concerns",
              },
            },
            required: ["request"],
          },
